...
export default function JoinCompanyClient({ slug }: { slug: string }) {
  const router = useRouter();
  const { register } = useAuth();
  const [isLoading, setIsLoading] = useState(false);

  // Use the slug prop directly
  const capitalizedSlug = slug.toUpperCase() + slug.slice(1);
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="space-y-2 text-center lg:text-left">
        ...
        <h1 className="text-3xl font-serif tracking-tight">
          Join {capitalizedSlug}
        </h1>
        <p className="text-sm text-muted-foreground font-light">
          Create your account to join the {capitalizedSlug} workspace on
          Presence.
        </p>
      </div>
