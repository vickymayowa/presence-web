...
import { useUsersQuery, useBranchesQuery, useAssignBranchMutation } from "@/lib/queries/presence-queries"
...
export default function TeamPage() {
    ...
    const handleAssignBranch = async (userId: string, branchId: string | null) => {
        try {
            await assignBranchMutation.mutateAsync({ userId, branchId })
            ...
            <div className="flex flex-col md:flex-row items-center gap-4">
                ...
                <div className="flex gap-2 w-full md:w-auto">
                    <Select value={branchFilter} onValueChange={setBranchFilter}>
                        <SelectTrigger className="h-12 w-[180px] rounded-xl border-border/40 bg-secondary/10">
                        ...
                        <Card key={member.id} className="group border-border/30 hover:border-primary/20 hover:shadow-lg hover:shadow-primary/5 transition-all duration-300 overflow-hidden">
                            <CardHeader className="relative pb-0">
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end" className="rounded-xl border-border/40 min-w-[200px]">
                                        <DropdownMenuLabel className="text-xs font-light text-muted-foreground">Actions</DropdownMenuLabel>
                                        ...
                                        {(currentUser.role === 'ceo' || currentUser.role === 'hr') && (
                                            <>
                                                ...
                                                <DropdownMenuLabel className="text-xs font-light text-muted-foreground">Assign Branch</DropdownMenuLabel>
                                                <DropdownMenuItem onClick={() => handleAssignBranch(member.id, null)}>
                                                    No Branch
                                                </DropdownMenuItem>
                                                {branches.map((b: any) => (
                                                    <DropdownMenuItem key={b.id} onClick={() => handleAssignBranch(member.id, b.id)}>
                                                        {b.name}
                                                    </DropdownMenuItem>
                                                ))}
                                                <DropdownMenuSeparator />
